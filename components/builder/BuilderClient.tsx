...
export default function BuilderClient({
  ...
  const [resolvedDensity, setResolvedDensity] = useState<Density>("roomy");
  const [measuring, setMeasuring] = useState<"roomy" | "normal" | "compact" | null>(null);
    ...
    (async () => {
      try {
        ...
        if (doc.title) setDocName(doc.title);
          ...
          doc.templateId === "slab" ||
          doc.templateId === "compact" ||
          doc.templateId === "editorial" ||
  ...
  useEffect(() => {
    if (!documentLoaded) return;
      ...
      if (!autoFit) {
      setResolvedDensity(density === "auto" ? "roomy" : density);
   ...
   const handleMeasure = useCallback(
    (height: number) => {
      ...
      const onePage = PAGE_PX[pageSize].h;
      const twoPages = PAGE_PX[pageSize].h * 2;

      // Fits within 2 pages. Accept this density.
      if (height <= twoPages) {

        if (
          height <= onePage * 0.8 &&
          (measuring === "compact" || measuring === "normal")
        ) {
          setMeasuring(measuring === "compact" ? "normal" : "roomy");
          return;
        }
      ...
      if (measuring === "roomy") {
        setMeasuring("normal");
      } else if (measuring === "normal") {
        setMeasuring("compact");
      } else {
        // Even compact overflows.
